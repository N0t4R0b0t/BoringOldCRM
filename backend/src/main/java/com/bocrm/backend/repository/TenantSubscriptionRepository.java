...
public interface TenantSubscriptionRepository extends JpaRepository<TenantSubscription, Long> {
    ...
    @Modifying
    @Query("UPDATE TenantSubscription s SET s.tokensUsedThisPeriod = s.tokensUsedThisPeriod + :tokens WHERE s.tenantId = :tenantId")
    int incrementTokensUsed(@Param("tenantId") Long tenantId, @Param("tokens") long tokens);
}
