...
export const InvoiceView: React.FC<InvoiceViewProps> = ({ id }) => {
  ...
  const { fields: fieldDefinitions } = useCustomFields('Invoice');
  const { fields: calcDefinitions } = useCalculatedFields('Invoice');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setInvoice(res.data);
        setCalcLoading(true);
        apiClient.evaluateCalculatedFields('Invoice', id)
    ...
    <div className="space-y-6 p-1">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{invoice.customerName || `Customer #${invoice.customerId}`}</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-4">Invoice Details</h3>
        <dl className="grid grid-cols-2 gap-4">
          {field('Customer ID', invoice.customerId)}
          {field('Customer', invoice.customerName)}
          {field('Order ID', invoice.orderId)}
          {field('Currency', invoice.currency)}
          {field('Payment Terms', invoice.paymentTerms)}
          {field('Due Date', invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : null)}
          {field('Created', invoice.createdAt ? new Date(invoice.createdAt).toLocaleDateString() : null)}
        </dl>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5">
        ...
        <dl className="grid grid-cols-3 gap-4">
          {field('Subtotal', invoice.subtotal != null ? `$${Number(invoice.subtotal).toFixed(2)}` : null)}
          {field('Tax Amount', invoice.taxAmount != null ? `$${Number(invoice.taxAmount).toFixed(2)}` : null)}
          <div>
            ...
            {fieldDefinitions.map((f) => {
                ...
                return (
                  <div key={f.key} className="col-span-2">
                    <CustomRecordFieldPicker
                      value={invoice.customFields?.[f.key]}
                      onChange={() => {}}
              ...
              return (
                <div key={f.key} className={f.type === 'richtext' ? 'col-span-2' : ''}>
                  ...
                  {f.type === 'richtext' ? (
                    <div className="mt-1 prose prose-sm dark:prose-invert max-w-none text-sm text-gray-900 dark:text-gray-100" dangerouslySetInnerHTML={{ __html: invoice.customFields?.[f.key] || '' }} />
                  ) : (
                    <div className="mt-1 text-sm text-gray-900 dark:text-gray-100">{renderFieldValue(invoice.customFields?.[f.key])}</div>
                  )}
                </div>
              );
            })}
      ...
      <Timeline entityType="Invoice" entityId={id} />
    </div>
  );
};
