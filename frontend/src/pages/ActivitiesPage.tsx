...
export const ActivitiesPage = () => {
  ...
  const { fields: customFields = [] } = useCustomFields('Activity');
  const { fields: calculatedFields = [] } = useCalculatedFields('Activity');
  ...
  // Initialize with createdAt and desc for activities
  useEffect(() => {
    listState.setSortBy('createdAt');
    listState.setSortOrder('desc');
  }, []);
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      const data = normalizePaged<Activity>(response.data);
      setActivities(data.content);
      listState.setTotalPages(data.totalPages || 1);
      listState.setTotalElements(data.totalElements || data.content.length);
      listState.setError('');
    } catch (err: any) {
      listState.setError(err.response?.data?.message || 'Failed to fetch activities');
      setActivities([]);
    } finally {
  ...
  useEffect(() => {
    if (pendingFilters?.entityType === 'Activity') {
  ...
  const baseColumns: ColumnDefinition[] = [
    {
      ...
      sortable: true,
      filterType: 'text',
      render: (value: any, row: Activity) => {
        const subject = value || row.title || '—';
        return <span className="text-blue-600 font-medium">{subject}</span>;
      },
    },
    {
      key: 'type',
      ...
      render: (value: any) => {
        ...
        const color = getOptionBadgeColor(value, typeOptions);
        const colorClass = badgeColorMap[color] || badgeColorMap.blue;
        return (
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${colorClass}`}>
            {icons[value] || '•'} {getOptionLabel(value, typeOptions)}
          </span>
        );
      },
    },
  ...
  const customFieldColumns: ColumnDefinition[] = customFields
    ...
    .map((field) => ({
      ...
      sortable: false,
      filterType: 'text',
      render: (_value: any, row: Activity) => {
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <DataTable
          ...
          onBulkDelete={async (ids) => { await apiClient.bulkDeleteActivities(ids); fetchActivities(); }}
        />
