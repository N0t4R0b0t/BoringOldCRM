...
import { CustomRecordForm } from '../components/forms/CustomRecordForm';
import { CustomRecordView } from '../components/views/CustomRecordView';
...
export const CustomRecordsPage = () => {
  usePageTitle('CustomRecords');
  const [searchParams] = useSearchParams();
  const drawer = useDrawerState();
  const listState = useListPageState();
  const [customRecords, setCustomRecords] = useState<CustomRecord[]>([]);
  const [localError, setLocalError] = useState('');
  const { fields: customFields = [] } = useCustomFields('CustomRecord');
  const { fields: calculatedFields = [] } = useCalculatedFields('CustomRecord');
  ...
  const fetchCustomRecords = async () => {
    ...
    try {
      const response = await apiClient.listCustomRecords({
      ...
      const data = normalizePaged<CustomRecord>(response.data);
      setCustomRecords(data.content);
      listState.setTotalPages(data.totalPages || 1);
      listState.setTotalElements(data.totalElements || data.content.length);
      listState.setError('');
    } catch (err: any) {
      listState.setError(err.response?.data?.message || 'Failed to fetch customRecords');
      setCustomRecords([]);
    } finally {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this customRecord?')) {
      try {
        await apiClient.deleteCustomRecord(id);
        setCustomRecords(customRecords.filter((a) => a.id !== id));
  ...
  useEffect(() => {
    setHeaderActions(
      <NewEntityButton label="CustomRecord" onClick={drawer.handleNew} />
    );
  ...
  useEffect(() => {
    if (pendingFilters?.entityType === 'CustomRecord') {
  ...
  const customFieldColumns: ColumnDefinition[] = customFields
    ...
    .map((field) => ({
      ...
      render: (_value: any, row: CustomRecord) => {
        const v = row.customFields?.[field.key];
        return renderCustomFieldValue(v, field);
      },
    }));
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex-1">
            <AdvancedSearch
              ...
              placeholder="Search by customRecord name..."
            />
          </div>
        </div>

        <DataTable
          tableId="customRecords"
          data={customRecords}
          columns={columns}
          ...
          emptyMessage="No customRecords found. Create your first customRecord to get started."
          onBulkDelete={async (ids) => { await apiClient.bulkDeleteCustomRecords(ids); fetchCustomRecords(); }}
        />
        ...
        <Drawer
          ...
          onDelete={() => {
            if (drawer.selectedId) handleDelete(drawer.selectedId);
          }}
          title="CustomRecord Details"
          mode="view"
          width="1020px"
        >
          {drawer.selectedId && (
            <CustomRecordView id={drawer.selectedId} />
          )}
        </Drawer>
        ...
        <Drawer
          ...
          title={drawer.selectedId ? 'Edit CustomRecord' : 'New CustomRecord'}
          mode="edit"
          width="1230px"
